...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    val r = right.filterAcc(p, new Empty)
    val lr = l.union(r)
    val inclElem = if (p(elem)) lr.incl(elem) else lr
    acc union inclElem
  }
...
/**
  def mostRetweeted: Tweet = if(this.right.isEmpty) elem else (right.mostRetweeted)
  **/
...
object GoogleVsApple {
  ...
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(t => google.exists(s => t.text.contains(s)))
